    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 flex flex-col md:flex-row gap-10">
        ...
        <aside class="w-full md:w-1/4">
            <div class="bg-white p-6 border border-gray-100 shadow-sm relative">
                ...
                <h3 class="text-xl font-serif text-luxury-dark mb-6 tracking-wide">Lọc Tìm Kiếm</h3>
                ...
                <form action="{{ route('products.index') }}" method="GET" class="space-y-8" id="filter-form">
                    ...
                    <!-- Sorting -->
                    <div>
                        <h4 class="text-xs font-bold text-gray-800 uppercase tracking-widest mb-4">Sắp Xếp Theo</h4>
                    ...
                    <!-- Categories Filter -->
                    <div>
                        <h4 class="text-xs font-bold text-gray-800 uppercase tracking-widest mb-4">Danh Mục</h4>
                        <div class="space-y-3">
                            <div class="flex items-center group">
                                ...
                                <label for="cat_all" class="ml-3 text-sm text-gray-600 group-hover:text-luxury-dark cursor-pointer transition-colors">Tất Cả Danh Mục</label>
                            </div>
                    ...
                    <!-- Brands Filter -->
                    <div>
                        <h4 class="text-xs font-bold text-gray-800 uppercase tracking-widest mb-4">Thương Hiệu</h4>
                        <div class="space-y-3">
                            <div class="flex items-center group">
                                ...
                                <label for="brand_all" class="ml-3 text-sm text-gray-600 group-hover:text-luxury-dark cursor-pointer transition-colors">Tất Cả Thương Hiệu</label>
                            </div>
                    ...
                    <!-- Price Filter -->
                    <div>
                        <h4 class="text-xs font-bold text-gray-800 uppercase tracking-widest mb-4">Khoảng Giá</h4>
                        <div class="flex items-center gap-2">
                            <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="Tối Thiểu ₫" class="w-full text-xs border-gray-300 focus:border-luxury-gold focus:ring-luxury-gold transition-colors py-2" min="0" step="100000">
                            <span class="text-gray-400">-</span>
                            <input type="number" name="max_price" value="{{ request('max_price') }}" placeholder="Tối Đa ₫" class="w-full text-xs border-gray-300 focus:border-luxury-gold focus:ring-luxury-gold transition-colors py-2" min="0" step="100000">
                        </div>
                    </div>
        ...
        <div class="w-full md:w-3/4">
            ...
            <div class="mb-10 pb-4 border-b border-gray-100 flex justify-between items-end">
                @if(request('search'))
                    <div>
                        <h2 class="text-2xl font-serif text-luxury-dark tracking-tight">Kết Quả Tìm Kiếm</h2>
                        <p class="mt-1 text-sm text-gray-500">Từ khóa: <span class="font-bold font-serif text-luxury-gold">"{{ request('search') }}"</span></p>
                    </div>
                @else
                    <div>
                        <h2 class="text-3xl font-serif text-luxury-dark tracking-tight">Bộ Sưu Tập</h2>
                        <p class="mt-2 text-xs uppercase tracking-widest text-gray-500 font-semibold">Những Chiếc Đồng Hồ Tinh Tế</p>
                    </div>
                @endif
                <div class="flex items-center gap-6">
                    ...
                    <div class="text-sm text-gray-500 hidden sm:block">
                        Hiển thị {{ $products->firstItem() ?? 0 }}-{{ $products->lastItem() ?? 0 }} trong {{ $products->total() }}
                    </div>
                </div>
            </div>
                    ...
                    @foreach($products as $product)
                        <div class="bg-white border border-gray-100 hover:border-luxury-gold hover:shadow-xl transition-all duration-300 group flex flex-col h-full">
                            <div class="relative w-full aspect-[3/4] overflow-hidden bg-gray-50 flex items-center justify-center">
                                ...
                                @else
                                    <div class="w-full h-full flex items-center justify-center text-gray-400 text-xs font-serif italic group-hover:scale-105 transition-transform duration-700">Không Có Hình Ảnh</div>
                                @endif
            ...
            @else
                <div class="bg-white p-16 border border-gray-100 shadow-sm text-center relative">
                    ...
                    <h3 class="text-2xl font-serif text-luxury-dark mb-2">Không Tìm Thấy Sản Phẩm</h3>
                    <p class="text-gray-500 text-sm mb-6">Chúng tôi không tìm thấy kết quả nào phù hợp với bộ lọc hiện tại của bạn. Vui lòng thử các tiêu chí khác.</p>
