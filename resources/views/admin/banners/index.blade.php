...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Banners</h2>
        <p class="text-sm text-gray-500 mt-1">Manage homepage slider and ad banners.</p>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 text-gray-700 text-sm border-b border-gray-200">
                    <th class="px-6 py-4 font-semibold w-32">Image</th>
                    <th class="px-6 py-4 font-semibold">Title & Link</th>
                    <th class="px-6 py-4 font-semibold text-center w-24">Sort Order</th>
            ...
            <tbody class="divide-y divide-gray-200 text-sm">
                ...
                <tr class="hover:bg-gray-50 transition-colors group">
                    ...
                    <td class="px-6 py-4">
                        ...
                        @else
                            <div class="text-xs text-gray-500 mt-1">No Link</div>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4 text-center">
                        <form action="{{ route('admin.banners.toggle', $banner->id) }}" method="POST" class="inline-block m-0 p-0">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="focus:outline-none relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 {{ $banner->is_active ? 'bg-indigo-600' : 'bg-gray-200' }}" title="{{ $banner->is_active ? 'Click to disable' : 'Click to enable' }}">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.banners.edit', $banner->id) }}" class="p-2 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors tooltip-trigger" title="Edit">
                ...
                <tr>
                    <td colspan="5" class="px-6 py-10 text-center text-gray-500">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-base font-medium text-gray-900 mb-1">No banners found</p>
                            <p class="text-sm text-gray-500 mb-4">Get started by creating a new banner.</p>
                            <a href="{{ route('admin.banners.create') }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-800">Add your first banner</a>
                        </div>
                    </td>
                </tr>
                @endforelse
