...
<div class="max-w-3xl">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Thêm Banner Mới</h2>
        <p class="text-sm text-gray-500 mt-1">Tải lên banner mới cho cửa hàng của bạn.</p>
    </div>

    <form action="{{ route('admin.banners.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
        @csrf
        
        <div>
            <label for="title" class="block text-sm font-medium text-gray-700 mb-1">Tiêu đề Banner <span class="text-red-500">*</span></label>
            <input type="text" id="title" name="title" value="{{ old('title') }}" class="block w-full border border-gray-300 rounded-lg p-2.5 text-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500" required>
            @error('title') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
        </div>

        <div>
            <label for="link_url" class="block text-sm font-medium text-gray-700 mb-1">URL Liên kết</label>
            <input type="url" id="link_url" name="link_url" value="{{ old('link_url') }}" placeholder="https://example.com/promo" class="block w-full border border-gray-300 rounded-lg p-2.5 text-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" class="block w-full border border-gray-300 rounded-lg p-2.5 text-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500">
        ...
        <div x-data="bannerImageUpload()">
            <label class="block text-sm font-medium text-gray-700 mb-1">Hình ảnh Banner <span class="text-red-500">*</span></label>
            
            <div x-show="preview" class="mb-4 relative rounded-lg border border-gray-200 overflow-hidden w-full max-w-lg aspect-[21/9]" style="display: none;">
            ...
            <div x-show="!preview">
                ...
                <p class="text-xs text-gray-500 mt-1">Kích thước khuyên dùng: 1920x820px, Tối đa 4MB.</p>
            </div>
        ...
        <div class="pt-4 flex justify-end gap-3 border-t border-gray-100">
            ...
            <button type="submit" class="px-5 py-2.5 border border-transparent rounded-lg text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-sm">Lưu Banner</button>
        </div>
    </form>
</div>
