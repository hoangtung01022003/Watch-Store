<x-customer>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <h1 class="text-3xl font-serif text-luxury-dark mb-8">Thanh toán</h1>
            ...
            <div class="flex flex-col lg:flex-row gap-8">
                ...
                <div class="w-full lg:w-2/3 space-y-8">
                    ...
                    <div class="bg-white border border-gray-100 shadow-sm p-6" id="address-container">
                        <h2 class="font-serif text-xl text-luxury-dark mb-6 border-b border-gray-100 pb-4">Địa chỉ giao hàng</h2>
                        ...
                        @if($addresses->count() > 0)
                                    ...
                                    <label class="flex items-start p-4 border border-gray-200 cursor-pointer hover:border-luxury-gold transition-colors">
                                        ...
                                        <div class="ml-4">
                                            ...
                                            @if($address->is_default)
                                                <span class="inline-block mt-2 text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded">Địa chỉ mặc định</span>
                                            @endif
                                        </div>
                                    </label>
                                @endforeach
                            ...
                            <label class="flex items-center mb-4 cursor-pointer">
                                ...
                                <span class="ml-2 text-sm text-luxury-dark uppercase tracking-widest">Giao hàng đến địa chỉ khác</span>
                            </label>
                        @else
                            <input type="hidden" name="use_new_address" value="1" id="use_new_address">
                            <p class="text-sm text-gray-500 mb-4">Vui lòng cung cấp địa chỉ giao hàng cho đơn hàng đầu tiên của bạn.</p>
                        @endif
                        ...
                        <div id="new_address_form" class="{{ ($addresses->count() > 0 && old('use_new_address') != '1') ? 'hidden' : '' }} space-y-4 pt-4 border-t border-gray-100">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm text-gray-700 mb-1">Tên người nhận *</label>
                            ...
                            <div>
                                <label class="block text-sm text-gray-700 mb-1">Địa chỉ cụ thể *</label>
                                <input type="text" name="full_address" value="{{ old('full_address') }}" class="w-full border-gray-300 focus:border-luxury-gold focus:ring-0" placeholder="Tên đường, tòa nhà, số nhà...">
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    <label class="block text-sm text-gray-700 mb-1">Tỉnh/Thành phố *</label>
                                    <input type="text" name="city" value="{{ old('city') }}" class="w-full border-gray-300 focus:border-luxury-gold focus:ring-0">
                                </div>
                                <div>
                                    <label class="block text-sm text-gray-700 mb-1">Quận/Huyện *</label>
                                    <input type="text" name="district" value="{{ old('district') }}" class="w-full border-gray-300 focus:border-luxury-gold focus:ring-0">
                                </div>
                                <div>
                                    <label class="block text-sm text-gray-700 mb-1">Phường/Xã *</label>
                    ...
                    <div class="bg-white border border-gray-100 shadow-sm p-6">
                        <h2 class="font-serif text-xl text-luxury-dark mb-6 border-b border-gray-100 pb-4">Phương thức thanh toán</h2>
                        <div class="space-y-4">
                            <label class="flex items-center p-4 border border-luxury-gold bg-gray-50 cursor-pointer">
                                ...
                                <div class="ml-4">
                                    <div class="font-semibold text-luxury-dark">Thanh toán khi nhận hàng (COD)</div>
                                    <div class="text-sm text-gray-500">Thanh toán khi bạn nhận được đơn hàng.</div>
                                </div>
                            </label>
                    ...
                    <!-- Order Note -->
                    <div class="bg-white border border-gray-100 shadow-sm p-6">
                        <h2 class="font-serif text-xl text-luxury-dark mb-6 border-b border-gray-100 pb-4">Ghi chú đơn hàng (Tùy chọn)</h2>
                        <textarea name="note" rows="3" class="w-full border-gray-300 focus:border-luxury-gold focus:ring-0 text-sm" placeholder="Ghi chú về đơn hàng của bạn, ví dụ: ghi chú đặc biệt khi giao hàng.">{{ old('note') }}</textarea>
                    </div>
                ...
                <div class="w-full lg:w-1/3">
                    <div class="bg-white border border-gray-100 shadow-sm p-6 sticky top-6">
                        <h2 class="font-serif text-xl text-luxury-dark mb-6 border-b border-gray-100 pb-4">Đơn hàng của bạn</h2>
                            ...
                            @foreach($cart as $id => $item)
                                <div class="flex justify-between items-start border-b border-gray-50 pb-4 last:border-0 last:pb-0">
                                    <div class="pr-4">
                                        ...
                                        <div class="text-xs text-gray-500 mt-1">SL: {{ $item['quantity'] }}</div>
                                    </div>
